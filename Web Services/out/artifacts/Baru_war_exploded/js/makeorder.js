...
function setPreferDriver(search,id,pick,destination){
    if(search != ""){
        console.log(search);
        var xmlhttp;
        ...
        xmlhttp.open("GET", "testbackend/driver.jsp?search=" + search + "&id_active="+ id +"&destination="+destination+"&pick="+pick, true);
        xmlhttp.send();
    } else {
        document.getElementById("preffered-driver").innerHTML = "<div class='head-title driver-list content-font-segoe'> <p class='posisi-tengah'>Nothing to Display :(</p></div>"
    }
...
function setOtherDriver(search,id,pick,destination){
        var xmlhttp;
        if (window.XMLHttpRequest) {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        ...
        xmlhttp.open("GET", "testbackend/driver.jsp?cari=" + search + "&id_active="+ id +"&destination="+destination+"&pick="+pick, true);
        xmlhttp.send();
}
...
function iChoose(id_driver) {
    ...
    xmlhttp.open("GET", "testbackend/driver.jsp?get_info=" + id_driver, true);
    xmlhttp.send();
}
